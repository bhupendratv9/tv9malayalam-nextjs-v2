...
const MENU_API_BASE = (process.env.MENU_API_BASE_URL || "").replace(/\/$/, "");

function menuEndpoint(slug) {
  if (!MENU_API_BASE || !slug) return "";
  return `${MENU_API_BASE}/${slug}`;
}

export const ENDPOINTS = {
  MAIN_NAV_MENU: menuEndpoint(process.env.MAIN_NAV_MENU_SLUG),
  TRENDING_MENU: menuEndpoint(process.env.TRENDING_MENU_SLUG),
  TOP_CITIES_MENU: menuEndpoint(process.env.TOP_CITIES_MENU_SLUG),
  BOTTOM_NAV_MENU: menuEndpoint(process.env.BOTTOM_NAV_MENU_SLUG),
};
...
// Homepage breaking ticker (CMS `breaking-strip-widget` has no endpoint)
export const HOME_BREAKING_NEWS_TICKER_API =
  "https://alphapublish.tv9hindi.com/tv9malayalam/pagecategory/sections/home-breaking-news-ticker";
