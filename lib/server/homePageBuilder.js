...
function rewritePermalink(url) {
  ...
  return url
    .replace(
      /^https?:\/\/(www|app|alpha|alphapublish)\.tv9(hindi|tamil|telugu|marathi|kannada|bangla)(news)?\.com/,
      SITE_URL
    )
    .replace(/\/tv9malayalam-nextjs\/tv9malayalam-nextjs\//g, "/tv9malayalam-nextjs/");
}
  ...
  if (pageDetailData && isBreadcrumbWidget && (pageDetailData.breadcrumb || pageDetailData.data?.breadcrumb)) {
    ...
    try {
      ...
      if (effectiveEndpointType === "detail") {
        ...
        items = mapItemsForPublic(Array.isArray(fetchedData) ? fetchedData : []);
      }
    } catch (e) {
