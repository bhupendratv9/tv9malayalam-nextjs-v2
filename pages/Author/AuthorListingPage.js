...
export default function AuthorListingPage({ pageData, queryParams = {} }) {
...
export async function getServerSideProps({ query }) {
  const nameSlug = query?.nameSlug || query?.authorSlug || "";
  if (!nameSlug) return { notFound: true };

  return getPageProps(PAGE_IDS.AUTHOR_LISTING, {
    query: { ...query, nameSlug, author: nameSlug, category: "author" },
  });
}
