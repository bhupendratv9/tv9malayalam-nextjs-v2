...
export default function AnchorDetailPage({ pageData, queryParams = {} }) {
...
export async function getServerSideProps({ query }) {
  const anchorSlug = query?.anchorSlug || "";
  if (!anchorSlug) return { notFound: true };

  return getPageProps(PAGE_IDS.ANCHOR_DETAIL, {
    query: { ...query, anchorSlug, category: "anchor" },
  });
}
