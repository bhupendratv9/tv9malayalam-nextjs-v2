...
import styles from "./AqiScale.module.css";
...
export default function AqiScaleWidget({
  ...
  const heading =
    dataConfig.title || title || "காற்றின் தரக் குறியீட்டு அளவுகோல்";
  ...
  return (
    <div className={styles.AQIScaleWidget_Wrapper}>
      <div className={styles.container}>
        <div className={styles.custom_heading}>
          <h2 className={styles.h1}>{heading}</h2>
        </div>

        <ul className={styles.AQIScale_Thumbs}>
          {scaleItems.map((scale) => (
            <li key={scale.id} className={styles[scale.className]}>
              <div className={styles.AQInfo}>
                <strong>{scale.range}</strong>
                <small>AQI</small>
                <div className={styles.AQIType}>{scale.label}</div>
              </div>
            </li>
          ))}
        </ul>
