...
export default function CricketNewsTopNewsWidget({
  title = "",
  items = [],
  sectionUrl = "#",
  dataConfig = {},
  ...
  console.log("CricketNewsTopNewsWidget items:", items);

  if (!items || items.length === 0) return null;

  // Remaining items from index 2 onward — no image
  ...
  return (
    <div className="common_section topNews9">
      ...
      <div className="wrapper_section">
        {/* First figure (index 0) — big with image */}
        {items[0] && (() => {
          const row = items[0];
        ...
        {/* Second figure (index 1) — big with image */}
        {items[1] && (() => {
          const row = items[1];
          const url = row.url || row.permalink || "#";
          const titleText = decodeHtml(row.title || "");
