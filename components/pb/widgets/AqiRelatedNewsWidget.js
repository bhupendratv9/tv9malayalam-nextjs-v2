...
export default function AqiRelatedNewsWidget({ title = "", items = [], dataConfig = {} }) {
  const displayTitle =
     decodeHtml(dataConfig.title) || decodeHtml(title) || "सम्बंधित खबर";
  ...
  return (
    <div className="AQIRelatedNews_Widget">
      <div className="container">
        <div className="AQIHeading_Wrap">
          <h2 className="h1">{displayTitle}</h2>
        </div>

        <div className="AQIRelatedNews_Listing">
          {visibleNews.map((news) => {
            const newsTitle = decodeHtml(news.title);
            return (
              <figure key={news.id || news.post_id || news.url}>
                <a href={news.url} title={newsTitle}>
                  ...
                  <div className="cardTitle">{newsTitle}</div>
                </a>
              </figure>
            );
          })}
        ...
        {showLoadMore && (
          <div
            className="loadMoreBtn_v2 viewMoreBtn"
