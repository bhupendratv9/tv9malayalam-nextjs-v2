        ...
        <div className="imgCont">
          {author.image ? (
            <Image
              ...
              src={author.image}
              className="avatar avatar-170 photo"
              height={IMAGE_SIZE}
              width={IMAGE_SIZE}
              loading="lazy"
              unoptimized
...
export default function AuthorListWidget({
  ...
  const displayTitle = decodeHtml(dataConfig.title || title) || "ஆசிரியர்கள்";
  const viewMoreUrl = getViewMoreUrl({ view_more_link, dataConfig, sectionUrl, fallback: "/author" });
  ...
  return (
    <div className="authorsListing_Wrapper">
      ...
      <div className="authorsListing_Grid" id="category-ajax-posts">
        {authors.map((author) => (
