...
/**
 * Build breadcrumb HTML from article data.
 * Priority: article.breadcrumb array (from API) → fallback to taxonomies.category
 */
function buildBreadcrumbAmp(article, siteUrl, siteSettings = {}) {
  const homeTitle = siteSettings?.breadcrumb_home_title || "Home";
  const breadcrumbArray = getValue(article, "breadcrumb", []);

  // Use API breadcrumb array if available
  if (Array.isArray(breadcrumbArray) && breadcrumbArray.length > 0) {
    const items = breadcrumbArray.map((item) => {
      const name = item?.name || "";
      const url = item?.url || "#";
      return `<span><a href="${esc(url)}" title="${esc(name)}">${esc(name)}</a></span>`;
    });
    return `<div id="breadcrumbs" class="breadcrumb">${items.join("")}</div>`;
  }

  // Fallback: build from taxonomies.category
  const categories = getValue(article, ["taxonomies", "category"], []);
  let breadcrumbItems = `<span><a href="${esc(siteUrl)}">${esc(homeTitle)}</a></span>`;
...
export function renderDetailMainContentAmp(section, queryParams, meta, siteSettings = {}) {
  ...
  // Follow Us URLs (from siteSettings first, then config/dataConfig fallback)
  const liveTvUrl = String(
    config?.live_tv_url || dataConfig?.live_tv_url || siteSettings?.live_tv_url || ""
  );
  const youtubeUrl = String(
    config?.youtube_url || dataConfig?.youtube_url || siteSettings?.youtube_url || ""
  );
  const facebookUrl = String(
    config?.facebook_url || dataConfig?.facebook_url || siteSettings?.facebook_url || ""
  );
  const twitterUrl = String(
    config?.twitter_url || dataConfig?.twitter_url || siteSettings?.twitter_url || ""
  );
  ...
  // Dynamic breadcrumb
  const breadcrumbHtml = buildBreadcrumbAmp(article, siteUrl, siteSettings);
